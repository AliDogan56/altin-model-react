...
## Teknik analiz backend taşıması geri alındı (2026-09-07)

6 Eylül'de destek/direnç, pivot, gösterge, trend ve momentum hesapları market-service'e
taşınmıştı (`90c4f4c`, `GET /v1/market/xau/technical`, referans = gün içi vadeli kapanış).
7 Eylül'de ölçüm, günlük serinin spot seviyesinde, 5 dk vadeli serinin ise ~%1 üstünde
olduğunu gösterdi; referans seviyelerin üstünde kalıyor, kullanıcı "seviyeler spotun
gerisinde" diye bildirdi. Kullanıcı kararıyla taşıma ve ona bağlı üç arayüz commit'i
(`6586905` üçüncü taraf adları + mobil alt sekme çubuğu, `2c7301b` merdiven taşması,
`c270c83` genel bakış yerleşimi) **tamamen geri alındı**; pano yine bu dosyada anlatılan
eski akışla çalışır: pivot/S-R/trend/gösterge/işlem bölgeleri tarayıcıda, referans canlı
Harem spotu, momentum `GET /v1/market/xau/momentum`. Taşımadan yalnız iki TA-dışı düzeltme
tutuldu: `xau_history` yedeğe düşerken `ValueError`'ı da yakalar (200 + HTML gövde) ve
`csp.test.ts` Node tipleri olmadan derlenir (`?raw` + Web Crypto). Taşımanın ölçümleri ve
sebepleri git geçmişinde (`90c4f4c` içindeki `docs/technical/VALIDATION.md`).
