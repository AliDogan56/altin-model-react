...
function IndicatorsSection({ focus }: { focus?: string }) {
  ...
  return (
    ...
    <small>Günlük XAU/USD verisinden hesaplanır. Göstergenin bulunduğu durum yazılır; bilinçli olarak alım-satım kararı üretilmez — aşırı alım bölgesi yükselişin biteceği anlamına gelmez, güçlü trendlerde gösterge uzun süre orada kalabilir.</small></div></div>
    ...
    <span>MA{a.n}<em>EMA {a.ema==null?'—':money(a.ema)}</em></span>
    <b>{a.sma==null?'—':money(a.sma)}</b>
    <i className={`tech-state ${a.sma!=null&&a.price>=a.sma?'up':'down'}`}>{a.sma==null?'Veri yetersiz':a.price>=a.sma?'Fiyat üstünde':'Fiyat altında'}</i>
    </div>)}
    </div>
    </section>
    </Collapsible>
