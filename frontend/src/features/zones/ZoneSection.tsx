...
function ZoneSection({ focus }: { focus?: string }) {
  const { capital, setCapital, riskPct, setRiskPct, zones } = useDashboard();
  const { buy, sell, stop, entry, units } = zones;
  return (
    ...
    <div className="position">Örnek azami pozisyon <b>{units.toFixed(3)} PAXG · {money(units*entry)}</b></div></section>
    </Collapsible>
  );
}
